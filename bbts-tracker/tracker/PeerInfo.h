#ifndef BBTS_TRACKER_PEER_INFO_H_
...
public:
    ...
    void SetStatus(tracker::Status::type status) {
        thrift_peer.__set_status(status);
    };
    void SetDownloaded(uint64_t downloaded) {
        thrift_peer.__set_downloaded(downloaded);
    };
    void SetUploaded(uint64_t uploaded) {
        thrift_peer.__set_uploaded(uploaded);
    };
    void SetLeft(uint64_t left) { thrift_peer.__set_left(left); };
    void SetExpireTime(uint64_t expire_time) {
        thrift_peer.__set_expire_time(expire_time);
    };
    void SetTimeStamp(uint64_t timestamp) {
        thrift_peer.__set_timestamp_start(timestamp);
    };
    void SetWantNumber(uint32_t want_number) {
        thrift_peer.__set_want_number(want_number);
    };
    void SetIsSetRemote(bool is_set_remote) {
        this->is_set_remote = is_set_remote;
    };
    void SetIsSeed(bool is_seed) { thrift_peer.__set_is_seed(is_seed); };
    void SetTrackerId(const std::string &tracker_id) {
        thrift_peer.__set_tracker_id(tracker_id);
    }
...
#endif // BBTS_TRACKER_PEER_INFO_H_
