...
void StatusManager::UpdateData() {
  BLOG(info) << "UpdateData thread created";
  ...
  while (!terminated_) {
    ...
    if (last_timestamp != timestamp && timestamp % update_period_ == 0) {
      ...
      BLOG(info) << "Update monitor status: " << last_timestamp;
    }
    nanosleep(&unit, NULL);
  }
  BLOG(info) << "UpdateData thread exit";
}
...
bool StatusManager::RegisterItem(const string& name, ItemType type) {
  ...
  if (item_map_.find(name) != item_map_.end()) {
    BLOG(warning) << name << " has already been registered";
    return false;
  }
  ...
  switch (type) {
    ...
    default:
      BLOG(warning) << "type: " << type << " can't be recognized";
      return false;
  }
...
bool StatusManager::UnregisterItem(const string& name) {
  ...
  if (item_it == item_map_.end()) {
    BLOG(warning) << name << " has not been registered";
    return false;
  }
...
bool StatusManager::GetItem(const string &key, shared_ptr<Item> *item) const {
  ...
  if (iter == item_map_.end()) {
    BLOG(warning) << key << " not registered";
    return false;
  }
  if (iter->second->type() != Item::type_) {
    BLOG(warning) << key << " doesn't have type " << Item::type_;
    return false;
  }
...
bool StatusManager::IncreaseItemCounting(const string& key, int64_t increasement) {
  if (increasement < 1) {
    BLOG(warning) << "Invalid increasement " << increasement;
    return false;
  }
